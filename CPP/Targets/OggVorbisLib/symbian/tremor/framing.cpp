...
static ogg_reference *_fetch_ref(ogg_buffer_state *bs){
  ...
  if(bs->unused_references){
    oggref=bs->unused_references;
    bs->unused_references=oggref->next;
  }else{
    /* allocate a new reference */
    oggref=(ogg_reference*)_ogg_malloc(sizeof(*oggref));
  }
...
static ogg_reference *ogg_buffer_alloc(ogg_buffer_state *bs,long bytes){
  ...
  ogg_reference *oggref=_fetch_ref(bs);
  oggref->buffer=ob;
  return oggref;
}

/* enlarge the data buffer in the current link */
static void ogg_buffer_realloc(ogg_reference *oggref,long bytes){
  ogg_buffer    *ob=oggref->buffer;
...
static void _ogg_buffer_mark_one(ogg_reference *oggref){
  oggref->buffer->refcount++;
}

/* increase the refcount of the buffers to which the reference points */
static void ogg_buffer_mark(ogg_reference *oggref){
  while(oggref){
    _ogg_buffer_mark_one(oggref);
    oggref=oggref->next;
  }
}
...
static ogg_reference *ogg_buffer_sub(ogg_reference *oggref,long begin,long length){
  ogg_reference *ret=0,*head=0;

  /* walk past any preceeding fragments we don't want */
  while(oggref && begin>=oggref->length){
    begin-=oggref->length;
    oggref=oggref->next;
  }
  ...
  while(oggref && length){
    ogg_reference *temp=_fetch_ref(oggref->buffer->ptr.owner);
    if(head)
      head->next=temp;
    else
      ret=temp;
    head=temp;
    head->buffer=oggref->buffer;    
    head->begin=oggref->begin+begin;
    head->length=length;
    if(head->length>oggref->length-begin)
      head->length=oggref->length-begin;
...
ogg_reference *ogg_buffer_dup(ogg_reference *oggref){
  ogg_reference *ret=0,*head=0;
  /* duplicate the reference chain; increment refcounts */
  while(oggref){
    ogg_reference *temp=_fetch_ref(oggref->buffer->ptr.owner);
    ...
    head=temp;
    head->buffer=oggref->buffer;    
    head->begin=oggref->begin;
    head->length=oggref->length;
    oggref=oggref->next;
  }
...
static ogg_reference *ogg_buffer_split(ogg_reference **tail,
  ...
  ogg_reference *oggref=*tail;

  while(oggref && pos>oggref->length){
    pos-=oggref->length;
    oggref=oggref->next;
  }
  ...
  if(!oggref || pos==0){
    ...
    if(pos>=oggref->length){
      /* exact split, oggref off the end? */
      if(oggref->next){
      ...
      long lengthA=pos;
      long beginB=oggref->begin+pos;
      long lengthB=oggref->length-pos;
      
      /* make a new reference to tail the second piece */
      *tail=_fetch_ref(oggref->buffer->ptr.owner);
      
      (*tail)->buffer=oggref->buffer;
      (*tail)->begin=beginB;
      (*tail)->length=lengthB;
      (*tail)->next=oggref->next;
      _ogg_buffer_mark_one(*tail);
      if(head && oggref==*head)*head=*tail;    
      
      /* update the first piece */
      oggref->next=0;
      oggref->length=lengthA;
...
static void ogg_buffer_release_one(ogg_reference *oggref){
  ogg_buffer *ob=oggref->buffer;
  ...
  oggref->next=bs->unused_references;
  bs->unused_references=oggref;
...
static void ogg_buffer_release(ogg_reference *oggref){
  while(oggref){
    ogg_reference *next=oggref->next;
    ogg_buffer_release_one(oggref);
    oggref=next;
  }
}

static ogg_reference *ogg_buffer_pretruncate(ogg_reference *oggref,long pos){
...
static ogg_reference *ogg_buffer_walk(ogg_reference *oggref){
  if(!oggref)return NULL;
  while(oggref->next){
    oggref=oggref->next;
  }
  return(oggref);
...
static int oggbyte_init(oggbyte_buffer *b,ogg_reference *oggref){
  memset(b,0,sizeof(*b));
  if(oggref){
    b->ref=b->baseref=oggref;
    b->pos=0;
    b->end=b->ref->length;
    b->ptr=b->ref->buffer->data+b->ref->begin;  
    return 0;
...
unsigned char *ogg_sync_bufferin(ogg_sync_state *oy, long bytes){
  ...
  */
  ...
  {
    ogg_reference *anew=(ogg_reference*)ogg_buffer_alloc(oy->bufferpool,bytes);
    oy->fifo_head->next=anew;
    oy->fifo_head=anew;
  }
  return oy->fifo_head->buffer->data;
}
...
static ogg_uint32_t _checksum(ogg_reference *oggref, int bytes){
  ogg_uint32_t crc_reg=0;
  int j,post;

  while(oggref){
    unsigned char *data=oggref->buffer->data+oggref->begin;
    post=(bytes<oggref->length?bytes:oggref->length);
    for(j=0;j<post;++j)
